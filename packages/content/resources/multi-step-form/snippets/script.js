(function () {
  ...
  const validators = {
    ...
    1: () => {
      ...
      let valid = true;
      clearErrors();
      if (!first.value.trim()) { setError(first, "firstName-error", "First name is required."); valid = false; }
      if (!last.value.trim())  { setError(last,  "lastName-error",  "Last name is required.");  valid = false; }
      return valid;
    },
    2: () => {
  ...
  function collectStepData(step) {
    switch (step) {
      ...
      case 2:
        ...
        formData.teamSize   = (document.querySelector('input[name="teamSize"]:checked') || {}).value || "";
  ...
  function buildReview() {
    const sections = [
      ...
      },
      {
        ...
        step:  1,
        rows: [
          { label: "Full Name", value: [formData.firstName, formData.lastName].filter(Boolean).join(" ") || "—" },
    ...
    reviewCard.innerHTML = sections.map((sec) => `
  ...
  startOver.addEventListener("click", () => {
    ...
    document.querySelectorAll(".step-panel").forEach((el, i) => {
      el.classList.remove("active", "slide-in-left", "slide-out-left", "slide-in-right", "slide-out-right");
      el.style.display = "";
